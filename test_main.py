...
async def cmd_task(message: Message, state: FSMContext):
    ...
    if len(args) > 1:
        ...
        task = {
            ...
            "responsible_id": int(parts[2]) if len(parts) > 2 and parts[2].isdigit() else None,
...
async def create_task(message: Message, parts: dict):
    ...
    try:
        ...
        responsible_id = int(parts["responsible_id"]) if len(parts) > 2 and parts["responsible_id"].isdigit() else user_data["user_id"]
        priority = int(parts["priority"]) if len(parts) > 3 and parts["priority"].isdigit() else 1
